        ...
        w-auto absolute top-0 left-0 right-0 opacity-100 z-10'></div>
                <div className='relative opacity-100 z-20 grid items-center nike-container justify-items-center'>
                    <div className='grid items-center justify-items-center mt-28 md:mt-24'>
                        <h1 className='text-6xl lg:text-5xl md:text-4xl sm:text-3xl xsm:text-2xl
                        font-extrabold filter drop-shadow-sm text-slate-200'>{title}</h1>
                        <h1 className='text-6xl lg:text-5xl md:text-4xl sm:text-3xl xsm:text-2xl
                        font-extrabold filter drop-shadow-sm text-slate-200'>{subtitle}</h1>
                        <button className='button-theme bg-slate-200 shadow-slate-200
                        rounded-xl my-5'>{btntext}</button>
                        <div className='grid items-center gap-5 md:gap-3 absolute top-[33vh] left-[11%] 
                        xl:left-0 w-auto h-auto'>
                            {
                                videos?.map((val, i) => (
                                    <Clips
                                        key={i}
...
<div className='relative h-auto w-auto flex flex-col'>
        ...
        w-auto absolute top-0 left-0 right-0 opacity-100 z-10'>
                <div className='relative opacity-100 z-20 grid items-center nike-container justify-center'>
                    <h1>{title}</h1>
                    <h1>{subtitle}</h1>
